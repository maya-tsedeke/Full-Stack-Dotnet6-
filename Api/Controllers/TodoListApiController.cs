...
namespace Api.Controllers
{
    ...
    public class TodoListApiController : BaseController
    {
        ...
        [HttpGet]
        [Route("todos/status={status}")]
        public async Task<IActionResult> GetByStatusId(string sort = "Id", bool desc = true, int status = 0)
        {
            try
            {
                var toDo = await _todoService.GetByStatus(sort, desc, status);
                if (toDo == null)
                    return NotFound();
